...
Theorem max_nat_largest : forall (x y : nat),
  max_nat x y = x <-> x >= y.
Proof.
  induction x; destruct y; split; intros; try lia; try reflexivity.
  - inversion H.
  - inversion H. rewrite H1. apply IHx in H1. lia.
  - simpl. assert (H': x >= y). { lia. }
...
Proof.
  ...
  - destruct tl1.
    + simpl in H. inversion H. subst. simpl.
      ...
      inversion H'.
      * subst; lia.
      * subst. rewrite max_nat_comm in H0. apply max_nat_largest in H0. lia.
...
Proof.
  ...
  - rewrite H0 in Heqz. rewrite max_nat_comm in Heqz.
    assert (y >= x). { apply max_nat_largest. lia. }
    lia.
Qed.
...
Proof.
  induction n; destruct f; split; intros; simpl; try lia.
  - exists tl_nil. reflexivity.
  - simpl in H. inversion H.
  - simpl in H. inversion H. inversion H0.
  - exists tl_nil. reflexivity.
  - simpl in H. assert (n >= treelist_length f) by lia.
  ...
  - simpl in H. destruct (point n f) eqn:e. destruct p. inversion H. inversion H0. subst.
    assert (n >= treelist_length f) by (apply IHn; exists t1; apply e). lia.
Qed.
...
Definition Z_7 := Zpos (xI (xI xH)).
Definition emp_1 : V := make_list_of 1 "A"%char.
Definition emp_2 : V := make_list_of 1 "B"%char.
Definition emp_3 : V := ["C"%char].
...
Definition ex_treelist : treelist :=
  tl_cons (node pos_one
    (tl_cons (val neg_one ["A"%char])
    (tl_cons (val pos_one ["B"%char]) tl_nil)))
  (tl_cons (final
    (tl_cons (val pos_six  ["Z"%char]) tl_nil))
  tl_nil).

Compute(get (make_cursor (Zneg (xO xH)) ex_treelist)).
Compute(get (make_cursor (Zpos xH) ex_treelist)).
Compute(get (make_cursor pos_six ex_treelist)).

Compute(make_cursor (Zpos (xI (xO xH))) ex_treelist).
Compute(get_treelist (
insert pos_one ["C"%char] (make_cursor pos_six ex_treelist))).

Compute(get (make_cursor (Zpos (xO (xI xH))) ex_treelist)).
...
Compute(first_cursor ex_treelist'').
(** Tests *)
